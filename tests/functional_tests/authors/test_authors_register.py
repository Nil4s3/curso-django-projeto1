...
class AuthorsRegisterTest(AuthorsBaseTest):
    ...
    def test_empty_first_name_error_message(self):
        def callback(form):
            first_name_field = self.get_by_polaceholder(form, 'Ex.: John')
    ...
    def test_empty_last_name_error_message(self):
        def callback(form):
            last_name_field = self.get_by_polaceholder(form, 'Ex.: Doe')
    ...
    def test_empty_username_error_message(self):
        def callback(form):
            username_field = self.get_by_polaceholder(form, 'Your username')
            username_field.send_keys(' ')
            username_field.send_keys(Keys.ENTER)
            form = self.get_form()
            self.assertIn('This field mus not be empty', form.text)
        self.form_field_test_with_callback(callback)

    def test_invalid_email_error_message(self):
        def callback(form):
            email_field = self.get_by_polaceholder(form, 'Your e-mail')
    ...
    def test_passwords_do_not_match(self):
        def callback(form):
            password1_field = self.get_by_polaceholder(
                form, 'Type your password')
            password2_field = self.get_by_polaceholder(
